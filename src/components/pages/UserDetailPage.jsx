...
const STATUS_CFG = {
  Active:   { dot:"var(--success)", text:"var(--success)", bg:"var(--success)/10", label:"Active"   },
  Inactive: { dot:"var(--muted-foreground)", text:"var(--muted-foreground)", bg:"var(--muted)", label:"Inactive" },
  Pending:  { dot:"var(--warning)", text:"var(--warning)", bg:"var(--warning)/20", label:"Pending"  },
};
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-3 gap-4">
        {stats.map(s=>(
          <div key={s.label} className="bg-card dark:bg-card border border-border dark:border-border rounded-[10px] p-4 flex items-center gap-3 shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
            ...
            <div>
              <p className="text-xl font-bold text-foreground dark:text-foreground leading-none">{s.value}</p>
              <p className="text-xs text-muted-foreground dark:text-muted-foreground mt-1">{s.label}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-card dark:bg-card border border-border dark:border-border rounded-[10px] overflow-hidden shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
        {[
          ...
          <div key={i} className="flex items-center justify-between px-5 py-3.5 border-b last:border-0 border-border dark:border-border">
            <span className="text-sm font-medium text-muted-foreground w-32">{row.label}</span>
            <span className="flex-1 text-sm text-muted-foreground dark:text-muted-foreground text-right">{row.value}</span>
          </div>
        ))}
      </div>
    </div>
...
function AccessRolesTab({ user, onRoleChange }){
  ...
  const ROLE_CFG_FULL = {
    "Admin":       { bg:"var(--primary)", text:"var(--primary-foreground)", desc:"Full access to all features except settings" },
    "Member":      { bg:"var(--border)", text:"var(--primary-foreground)", desc:"View-only access, can use agents and flows" },
    "Super Admin": { bg:"var(--chart-chart-4)", text:"var(--primary-foreground)", desc:"Unrestricted access to all platform features" },
  };
      ...
      <div className="bg-card dark:bg-card border border-border dark:border-border rounded-[10px] overflow-hidden shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
        ...
        <div className="p-4 flex flex-col gap-2">
          {ROLES.map(r=>{
            ...
            return(
              <label key={r} className={`flex items-center gap-3 p-3 rounded-[8px] border cursor-pointer transition-colors ${active?"border-border bg-primary/10 dark:bg-primary/20/20":"border-border dark:border-border hover:bg-muted dark:hover:bg-muted"}`}>
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {active && <span className="text-xs text-primary font-medium">Current</span>}
                  </div>
                  <p className="text-xs text-muted-foreground dark:text-muted-foreground mt-0.5">{cfg.desc}</p>
                </div>
              </label>
            );
          })}
          ...
          {Object.entries(grouped).map(([group,items])=>(
            <div key={group}>
              ...
              <div className="flex flex-col gap-1">
                {items.map(pm=>(
                  <label key={pm.key} className="flex items-center gap-3 px-3 py-2.5 rounded-[7px] hover:bg-muted dark:hover:bg-muted cursor-pointer transition-colors">
                    <span className={`size-4 rounded flex items-center justify-center border transition-colors flex-shrink-0 ${perms[pm.key]?"bg-primary border-border":"border-border dark:border-border"}`}>
                    ...
                    <input type="checkbox" checked={perms[pm.key]}
                      ...
                      className="sr-only"/>
                    <span className="text-sm text-muted-foreground dark:text-muted-foreground">{pm.label}</span>
                  </label>
                ))}
              </div>
            </div>
      ...
      <div className="bg-card dark:bg-card border border-border dark:border-border rounded-[10px] overflow-hidden shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
        <div className="px-5 py-3.5 border-b border-border dark:border-border flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">{userGroups.length} group{userGroups.length!==1?"s":""}</p>
          </div>
        </div>
        {userGroups.length>0 ? (
          <div className="divide-y divide-muted dark:divide-[#334155]">
            {userGroups.map(g=>(
              <div key={g.id} className="flex items-center justify-between px-5 py-3.5">
                <div className="flex items-center gap-3">
                  <div className="size-9 rounded-[8px] bg-primary/10 dark:bg-primary/20/30 flex items-center justify-center flex-shrink-0">
                    <Users size={16} className="text-primary"/>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-foreground dark:text-foreground">{g.name}</p>
                    <p className="text-xs text-muted-foreground">{g.members} members · {g.desc}</p>
                  </div>
                </div>
                <button onClick={()=>onGroupChange("remove",g.id)}
                  className="inline-flex items-center gap-1.5 h-7 px-2.5 rounded-[6px] border border-border dark:border-border text-xs font-medium text-muted-foreground dark:text-muted-foreground hover:border-destructive/30 hover:text-destructive hover:bg-destructive/10 transition-colors">
                  <X size={11}/> Remove
                </button>
              </div>
            ))}
          ...
          <div className="flex flex-col items-center gap-2 py-10">
            <Users size={28} className="text-foreground dark:text-muted-foreground"/>
        ...
        <div className="bg-card dark:bg-card border border-border dark:border-border rounded-[10px] overflow-hidden shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
          ...
          <div className="divide-y divide-muted dark:divide-[#334155]">
            {otherGroups.map(g=>(
              <div key={g.id} className="flex items-center justify-between px-5 py-3.5">
                <div className="flex items-center gap-3">
                  <div className="size-9 rounded-[8px] bg-muted dark:bg-border flex items-center justify-center flex-shrink-0">
                ...
                <button onClick={()=>onGroupChange("add",g.id)}
                  className="inline-flex items-center gap-1.5 h-7 px-2.5 rounded-[6px] border border-border dark:border-border text-xs font-medium text-muted-foreground dark:text-muted-foreground hover:border-border hover:text-primary hover:bg-primary/10 transition-colors">
                  + Add
                </button>
              </div>
            ))}
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border dark:border-border bg-background/40">
            <th className="px-5 py-3 text-left text-xs font-semibold text-muted-foreground dark:text-muted-foreground uppercase tracking-[0.05em]">Date & Time</th>
            <th className="px-5 py-3 text-left text-xs font-semibold text-muted-foreground dark:text-muted-foreground uppercase tracking-[0.05em]">Activity</th>
            <th className="px-5 py-3 text-left text-xs font-semibold text-muted-foreground dark:text-muted-foreground uppercase tracking-[0.05em]">Initiated By</th>
          </tr>
        </thead>
        <tbody>
          {ACTIVITY_LOG.map(row=>(
            <tr key={row.id} className="border-b last:border-0 border-border dark:border-border hover:bg-muted dark:hover:bg-muted/30 transition-colors">
              <td className="px-5 py-3.5 text-xs text-muted-foreground whitespace-nowrap">{row.date}</td>
              <td className="px-5 py-3.5 text-sm font-medium text-muted-foreground dark:text-muted-foreground">{row.activity}</td>
              <td className="px-5 py-3.5 text-sm text-muted-foreground dark:text-muted-foreground">{row.by}</td>
            </tr>
          ))}
        </tbody>
      </table>
        ...
        <div className="flex flex-col flex-1 min-w-0">
          <AppHeader onNavigate={onNavigate}>
            <nav className="flex items-center gap-1.5 text-sm ml-1">
              <button onClick={()=>onNavigate("users-list")} className="text-muted-foreground dark:text-muted-foreground hover:text-foreground dark:hover:text-foreground transition-colors">
                User Management
              </button>
              ...
              <ChevronRight size={13} className="text-foreground dark:text-muted-foreground"/>
              <span className="text-foreground dark:text-foreground font-medium">{user.name}</span>
            </nav>
          </AppHeader>
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        {groupNames(user.groups).map(g=>(
                          <span key={g.id} className="text-xs text-muted-foreground dark:text-muted-foreground">{g.name}</span>
                        ))}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <Button variant="outline"
                      ...
                      className="gap-1.5 h-9 text-sm font-medium border-border dark:border-border">
        ...
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-base">
              <span className={`size-8 rounded-full flex items-center justify-center flex-shrink-0 ${user.status==="Active"?"bg-destructive/10 dark:bg-card":"bg-success/10 dark:bg-card"}`}>
                {user.status==="Active"
